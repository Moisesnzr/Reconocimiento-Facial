...
namespace ReconocimientoFacialDCU
{
    public class clsDA
    {
        ...
        public static bool GuardarImagen(string Nombre, string Telefono, string Correo, byte[] Imagen)
        {
            cnx.Close();
            cnx.Open();
            OleDbCommand cmd = new OleDbCommand("INSERT INTO ROSTROS (Nombre, Telefono, Correo, Imagen) Values ('" + Nombre + "', '" + Telefono + "', '" + Correo + "',?);", cnx);
